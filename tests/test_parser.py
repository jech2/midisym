...
def test_find_matching_inst():
    ...
    ret = check_exact_match_note_rate_fn(midi_infilling_fn, midi_ori_fn, midi_inpainted_fn)
...
def test_calculate_transcription_measure():
    from midisym.analysis.utils import calculate_transcription_measure, get_all_notes
    ...
    from midisym.analysis.utils import time_to_ticks
    time_seconds = 0.05
    offset_tolerance_ticks = time_to_ticks(time_seconds, midi_obj_ori.ticks_per_beat, midi_obj_ori.tempo_changes[0].tempo)
    metrics = calculate_transcription_measure(all_notes_ori, all_notes_inpainted, offset_tolerance_ticks)
    print(metrics)
    metrics2 = calculate_transcription_measure(all_notes_ori, all_notes_inpainted, offset_tolerance_ticks)
    print(metrics2)
